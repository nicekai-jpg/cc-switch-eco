...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".cc-switch-eco").join("settings.json")
}
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = cc_switch_eco_lib::get_claude_settings_path();
    ...
    let db_path = std::path::Path::new(&home_dir)
        .join(".cc-switch-eco")
        .join("cc-switch-eco.db");
    assert!(
        db_path.exists(),
        "switching provider should persist to cc-switch-eco.db"
    );
