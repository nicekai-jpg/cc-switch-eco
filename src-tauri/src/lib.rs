...
pub use app_store::clear_override_cache;
...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("ccswitche://") {
        return false;
    }
...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/cc-switch-eco.log）
            {
                ...
                let log_file_path = log_dir.join("cc-switch-eco.log");
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("cc-switch-eco".into()),
                            }),
                        ])
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 CC Switch Eco Eco 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.ccswitche.desktop/applications/cc-switch-eco-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/cc-switch-eco-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first ccswitche:// URL
                        }
                    }
                }
            });
            ...
            let mut tray_builder = TrayIconBuilder::with_id(tray::TRAY_ID)
                .tooltip("CC Switch Eco Eco Eco") // 鼠标悬停提示
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 ccswitche://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        format!(
            "从旧版本迁移配置时发生错误：\n\n{error}\n\n\
            您的数据尚未丢失，旧配置文件仍然保留。\n\
            建议回退到旧版本 CC Switch Eco Eco 以保护数据。\n\n\
            点击「重试」重新尝试迁移\n\
        ...
        format!(
            ...
            Consider rolling back to an older CC Switch Eco Eco version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            2) If you see “database version is newer”, please upgrade CC Switch Eco Eco\n\
