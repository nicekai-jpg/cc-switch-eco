...
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = CC_SWITCH_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated CC Switch Eco proxy port to {port}");
        }
    } else {
        let _ = CC_SWITCH_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized CC Switch Eco proxy port to {port}");
    }
}

/// 获取 CC Switch Eco 代理服务器的监听端口
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 CC Switch Eco 的端口时才返回 true
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 CC Switch Eco 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 CC Switch Eco 自己端口的 loopback 地址才返回 true
