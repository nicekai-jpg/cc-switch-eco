...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "cc-switch-eco-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "cc-switch-eco-sync",
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    s3Config?.remoteRoot ?? "cc-switch-eco-sync",
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "cc-switch-eco-sync";
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(s3Config.remoteRoot ?? "cc-switch-eco-sync");
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "cc-switch-eco-sync",
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: s3RemoteRoot.trim() || "cc-switch-eco-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "cc-switch-eco-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${s3RemoteRoot.trim() || "cc-switch-eco-sync"}/v2/db-v6/${s3Profile.trim() || "default"}`;
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <Input
                ...
                placeholder="cc-switch-eco-sync"
                className="text-xs flex-1"
                disabled={isLoading}
              />
            </div>
